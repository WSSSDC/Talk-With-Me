...
class _VisualizerState extends State<Visualizer> {
  ...
  Widget build(BuildContext context) {
    return Container(
      height: 110,
      child: StreamBuilder(
        stream: SessionHandler.status == TalkStatus.user_talking ? MicInput.noiseStream : Stream<LoadingStream>.periodic(Duration(milliseconds: 100), (x) => LoadingStream()..meanDecibel = (sin(x) + 1) * 60),
...
class Bar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      child: Card(
        color: SessionHandler.status == TalkStatus.user_talking ? Colors.black : Colors.black45,
      ),
    );
  }
}
